<?php
/**
 * Gestión de KPIs por Departamento
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($_POST['form_action'] === 'add') {
        if (!$archivo) { setFlashMessage('Debe seleccionar un archivo', 'danger'); }
        else {
            $stmt = $pdo->prepare("INSERT INTO kpis_departamento (nombre, descripcion, archivo, departamento_id, activo) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$nombre, $descripcion, $archivo, $departamento_id, $activo]);
            setFlashMessage('KPI agregado correctamente', 'success');
        }
    } elseif ($_POST['form_action'] === 'edit') {
        $sql = "UPDATE kpis_departamento SET nombre = ?, descripcion = ?, departamento_id = ?, activo = ?";
        $params = [$nombre, $descripcion, $departamento_id, $activo];
        if ($archivo) { $sql .= ", archivo = ?"; $params[] = $archivo; }
        $sql .= " WHERE id = ?"; $params[] = $_POST['id'];
        $pdo->prepare($sql)->execute($params);
        setFlashMessage('KPI actualizado correctamente', 'success');
    }
...
$kpis = $pdo->query("SELECT k.*, d.nombre as dept_nombre FROM kpis_departamento k LEFT JOIN departamentos d ON k.departamento_id = d.id ORDER BY d.nombre, k.nombre")->fetchAll();
?>
<!DOCTYPE html>
<html lang="es">
<head>
        ...
        <main class="main-content">
            ...
            <div class="content-card">
                ...
                <div class="card-body">
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group"><label>Archivo (PDF, Excel, Word, PPT) <?php echo $action === 'add' ? '*' : ''; ?></label><input type="file" name="archivo" <?php echo $action === 'add' ? 'required' : ''; ?>></div>
            ...
            <div class="content-card">
                ...
                <div class="card-body"><div class="table-responsive"><table class="data-table">
                    <thead><tr><th>Nombre</th><th>Departamento</th><th>Archivo</th><th>Estado</th><th>Acciones</th></tr></thead>
                    <tbody>
                    <?php foreach ($kpis as $k): ?>
                    <tr>
                        <td><?php echo htmlspecialchars($k['nombre']); ?></td>
                        <td><?php echo htmlspecialchars($k['dept_nombre'] ?? '-'); ?></td>
                        <td><i class="fas fa-file" style="color:var(--admin-blue);"></i> <?php echo pathinfo($k['archivo'], PATHINFO_EXTENSION); ?></td>
